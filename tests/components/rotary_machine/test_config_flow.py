"""Test the Rotary Status config flow."""
...
async def test_config_flow(
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {"name": "My rotary_machine", "entity_id": input_sensor_entity_id},
    )
    await hass.async_block_till_done()

    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert result["title"] == "My rotary_machine"
    assert result["data"] == {}
    ...
    assert config_entry.title == "My rotary_machine"
...
async def test_options(hass: HomeAssistant, platform) -> None:
    ...
    state = hass.states.get(f"{platform}.my_rotary_machine")
